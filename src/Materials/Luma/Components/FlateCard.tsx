        ...
        >
            ...
            <div className="flex-1 flex flex-col p-6 lg:p-8" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex items-baseline gap-2 mt-2">
                    <p className="text-2xl lg:text-3xl font-bold text-gray-900">{price}</p>
                    <p className="text-sm text-gray-400 line-through">$99.00</p>
                    <span className="bg-green-100 text-green-700 text-xs font-semibold px-2 py-0.5 rounded-full ml-2">-22%</span>
                </div>

                <div className="flex items-center gap-4 mt-3">
                    <div className="flex items-center gap-2">
                        <Rating initialValue={rating} size={18} readonly={true} allowFraction={true} SVGclassName="inline-block" />
                        <span className="text-sm font-semibold text-gray-700">{rating}</span>
                    </div>
                    <div className="w-px h-4 bg-gray-300" />
                    <span className="text-sm text-gray-500">{reviewCount} Verified Reviews</span>
                </div>
                ...
                <div className="mt-6">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <button className="text-xs text-blue-600 hover:text-blue-700 font-medium" onClick={(e) => e.stopPropagation()}>Size Guide →</button>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {sizes.map((size) => (
                            <button
                                ...
                                className={`min-w-[48px] h-10 rounded-xl text-sm font-medium transition-all duration-300 ${selectedSize === size ? 'bg-blue-600 text-white shadow-lg shadow-blue-200 scale-105' : 'bg-white border-2 border-gray-200 text-gray-700 hover:border-blue-400 hover:shadow-md'}`}
                            >
                                {size}
                            </button>
                        ))}
                ...
                <div className="mt-5">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        {colors.map((color) => (
                            <button
                                key={color.label}
                                onClick={(e) => { e.stopPropagation(); setSelectedColor(color.label); }}
                                className={`group relative flex items-center gap-2 transition-all duration-300 ${selectedColor === color.label ? 'scale-105' : ''}`}
                            >
                                <div className={`w-9 h-9 rounded-full border-2 transition-all ${selectedColor === color.label ? 'border-blue-600 ring-2 ring-blue-200' : 'border-gray-200 group-hover:border-gray-400'}`} style={{ backgroundColor: color.code }} />
                                <span className={`text-xs font-medium transition-colors ${selectedColor === color.label ? 'text-blue-600' : 'text-gray-600'}`}>{color.label}</span>
                            </button>
                        ))}
                    </div>
                </div>
                ...
                <div className="mt-6 pt-2 flex flex-col gap-3">
                    <button onClick={handleAddToCart} disabled={!selectedSize || isAdding} className={`w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 disabled:from-gray-300 disabled:to-gray-400 disabled:cursor-not-allowed text-white font-semibold py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all duration-300 shadow-lg shadow-blue-200 hover:shadow-xl ${isAdding ? 'animate-pulse' : ''}`}>
                        <FaShoppingCart className="text-lg" />
                        {isAdding ? 'Adding...' : (selectedSize ? 'Add to Cart' : 'Select Size First')}
                    </button>

                    <div className="flex gap-3">
                        <button onClick={handleWishlist} className="flex-1 py-3 border-2 border-gray-200 rounded-xl hover:bg-gray-50 hover:border-blue-200 transition-all group flex items-center justify-center gap-2">
                            <FaHeart className={`${isInWishlist ? 'text-red-500' : 'text-gray-600 group-hover:text-red-500'} transition-colors`} />
                            <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900">{isInWishlist ? 'Added' : 'Wishlist'}</span>
                        </button>

                        <button onClick={handleCompare} className="flex-1 py-3 border-2 border-gray-200 rounded-xl hover:bg-gray-50 hover:border-blue-200 transition-all group flex items-center justify-center gap-2">
                            <FaExchangeAlt className={`${isInCompare ? 'text-blue-600' : 'text-gray-600 group-hover:text-blue-600'} transition-colors`} />
                            <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900">{isInCompare ? 'Compared' : 'Compare'}</span>
                        </button>
                    </div>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between">
                    ...
                    <a href="#" className="text-blue-600 hover:text-blue-700 text-xs font-medium flex items-center gap-1 group" onClick={(e) => e.stopPropagation()}>Learn More →</a>
                </div>
            </div>
        </div>
    );
