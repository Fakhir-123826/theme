...
//                 <Link
//                   to={data.url}
//                   className="main_game_section flex items-center gap-1 px-4 py-2 rounded-md hover:bg-gray-200 transition-colors"
//                 >
//                   {item}
//                   {data.hasChildren && <IoIosArrowDown size={14} />}
//                 </Link>
...
import { Menu, Search, ShoppingCart, X, Trash2, Plus, Minus, User, Heart, Package, MapPin, CreditCard, Edit, LogOut } from 'lucide-react';
import { IoIosArrowDown } from "react-icons/io";
import { Link } from 'react-router-dom';
import logo from '../../../assets/logo.svg';
import { useState } from 'react';
...
const Navbar = () => {
  ...
  // Menu data structure with updated URLs
  const menuData: Record<string, any> = {
    "What's New": { url: "/LumaHome/Whats_new", hasChildren: false },
    "Women": {
      url: "/LumaHome/Women_page",
      hasChildren: true,
      children: {
        "Tops": {
          url: "/LumaHome/Women_page/tops",
          items: ["Jackets", "Hoodies & Sweatshirts", "Tees", "Bras & Tanks"]
        },
        "Bottoms": {
          url: "/LumaHome/Women_page/bottoms",
          items: ["Pants", "Shorts"]
        }
      }
    },
    "Men": {
      url: "/LumaHome/Men_page",
      hasChildren: true,
      children: {
        "Tops": {
          url: "/LumaHome/Men_page/tops",
          items: ["Jackets", "Hoodies & Sweatshirts", "Tees", "Tanks"]
        },
        "Bottoms": {
          url: "/LumaHome/Men_page/bottoms",
          items: ["Pants", "Shorts"]
        }
      }
    },
    "Gear": { url: "/LumaHome/Gear_page", hasChildren: false },
    "Training": { url: "/LumaHome/Training_page", hasChildren: false },
    "Sale": { url: "/LumaHome/Sale_page", hasChildren: false }
  };

  // Account menu items for dropdown
  const accountMenuItems = [
    { label: "My Account", url: "/LumaHome/MyAccountPage", icon: User },
    { label: "My Orders", url: "/LumaHome/MyOrdersPage", icon: Package },
    { label: "My Wish List", url: "/LumaHome/MyWishListPage", icon: Heart, badge: wishlistItems.length },
    { label: "Address Book", url: "/LumaHome/AddressBookPage", icon: MapPin },
    { label: "Account Information", url: "/LumaHome/EditAccountInformationPage", icon: Edit },
    { label: "Stored Payment Methods", url: "/LumaHome/StoredPaymentMethods", icon: CreditCard },
    { label: "Shipping", url: "/LumaHome/ShippingPage", icon: Package },
    { label: "Review & Payments", url: "/LumaHome/ReviewAndPaymentsPage", icon: CreditCard },
  ];
        ...
        <div className="h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            {/* <div
              ...
              onMouseLeave={() => setHoveredMenu(null)}
            >
              <button className="flex items-center gap-1 px-4 py-2 rounded-md hover:bg-gray-100 transition-colors text-gray-700">
                <User size={18} />
              ...
              {hoveredMenu === "account" && (
                <div className="absolute right-0 top-full mt-1 bg-white shadow-xl border border-gray-100 rounded-xl min-w-[240px] z-50 overflow-hidden">
                  <div className="py-2">
            ...
            >
              ...
              {hoveredMenu === "account" && (
                <div className="absolute right-0 top-full mt-1 bg-white shadow-xl border border-gray-100 rounded-xl min-w-[260px] z-50 overflow-hidden">
                  <div className="py-2">
                    {/* My Account Section */}
                    <Link
                      to="/LumaHome/MyAccountPage"
                    ...
                    <Link
                      to="/LumaHome/EditAccountInformationPage"
                      className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors"
                      onClick={() => setHoveredMenu(null)}
                    >
                      <Edit size={16} className="text-gray-400" />
            ...
            {cartItems.length === 0 ? (
              ...
              <>
                  ...
                  {cartItems.map((item: any, index: any) => (
                    <div key={`${item.id}-${item.size}-${item.color}-${index}`} className="flex gap-4 p-3 bg-gray-50 rounded-xl hover:shadow-md transition-shadow">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <button onClick={() => handleUpdateQuantity(item, item.quantity - 1)} className="w-7 h-7 rounded-lg border border-gray-300 flex items-center justify-center hover:bg-gray-100 transition-colors">
                              <Minus size={12} />
                            </button>
                            <span className="text-sm font-medium w-8 text-center">{item.quantity}</span>
                            <button onClick={() => handleUpdateQuantity(item, item.quantity + 1)} className="w-7 h-7 rounded-lg border border-gray-300 flex items-center justify-center hover:bg-gray-100 transition-colors">
                          ...
                          <div className="text-right">
                            <p className="font-semibold text-gray-800">${(item.price * item.quantity).toFixed(2)}</p>
                            <button onClick={() => handleRemoveItem(item)} className="text-xs text-red-500 hover:text-red-600 mt-1">Remove</button>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="border-t p-5 bg-gray-50">
                  ...
                  <Link to="/LumaHome/ShippingPage">
                    <button className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3 rounded-xl font-semibold hover:from-blue-700 hover:to-purple-700 transition-all shadow-md">
                      Proceed to Checkout
                    </button>
                  </Link>

                  <button onClick={() => setIsCartOpen(false)} className="w-full text-center text-sm text-gray-500 hover:text-gray-700 mt-3">
                    Continue Shopping
                  </button>
                </div>
              </>
                ...
                {accountMenuItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {item.badge !== undefined && item.badge > 0 && (
                        <span className="bg-pink-500 text-white text-xs px-2 py-0.5 rounded-full">{item.badge}</span>
                      )}
                    </Link>
                  );
                })}
            ...
            <div className="space-y-4">
              {Object.entries(menuData).map(([item, data]) => (
                <Link
                  key={item}
                  to={data.url}
                  className="block text-lg font-medium text-gray-800 hover:text-blue-600 py-2 transition-colors"
                  onClick={() => setIsMenuOpen(false)}
                >
                  {item}
                </Link>
